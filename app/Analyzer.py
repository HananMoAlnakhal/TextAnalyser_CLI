...
#--------------------------------------------------
    def _displaySearch(self,searchResult):
        ...
        print()
        for X in searchResult:                                    #{ITALIC("Word Index:")} {BOLD(X[0][1])} 
            print(f'{ITALIC("Sentence Index:")} {BOLD(X[0][0])}\t \n"{X[1][0:X[2]]}{GREEN(ITALIC(UNDERLINED(X[1][X[2]:X[3]])))}{X[1][X[3]:len(X[1])]}"')
            print("-"*25)
########################################################################################
    ...
    def replaceWords(self,OldVal,NewVal):
        ...
        if OLdPos==None:
            ...
            for INX,((SenIndx,WordIndx),s,Bpos,Epos) in enumerate(OLdPos):
                if s==OLdPos[INX-1][1] and len(OLdPos)>1:#if it is the same as previous edited sentence then edit on the edited version not the original
                    Bpos+=len(oldNewDic[s.lower()])-len(s)
                    Epos+=len(oldNewDic[s.lower()])-len(s)
                    s=oldNewDic[s]
