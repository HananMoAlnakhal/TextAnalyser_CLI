...
from lang_models import POSITIVE_LEX,NEGATIVE_LEX,TRIE,Pred3Gram,Pred2Gram
...
if __name__=="__main__":
    ...
    posSentC=len([x for x in Report.sentiment_by_sentence if x>0])
    NatSentC=len([x for x in Report.sentiment_by_sentence if x==0])
    NegSentC=len([x for x in Report.sentiment_by_sentence if x<0])
    print("SentNum:",len(Report.sentiment_by_sentence))
    print("pos Sentences percentage:",round(posSentC/(posSentC+NegSentC) *100,1))
    print("nat Sentences count:",NatSentC)
    print("neg Sentences count:",NegSentC)
    print("-"*50)
    total=Report.sentiment["pos"]+Report.sentiment["neg"]
    print("TOTAL LEX:",total)
    print("pos lex:",Report.sentiment["pos"],"\t percentage :",Report.sentiment["pos"]/total)
    print("neg lex:",Report.sentiment["neg"],"\t percentage :",Report.sentiment["neg"]/total)