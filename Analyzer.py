...
class SmartTextAnalyzer():
    ...
    def __init__(self,TextInput="",nGram=3):
        self.text=TextInput
        self.sentences=[]
        self.words=[]
        self.ngram_size = nGram if nGram>=2 else 2
        self.ngram = {}
        if self.text!="":
...
#---------------------------------------------------------------------------------------
    def getWords(self):
        ...
        for ind,sentence in enumerate(self.sentences):
            expanded_sentence = self.expand_contractions(sentence, __class__.all_contractions)
            wordsInSentence = []
            ...
            self.add_ngram(wordsInSentence)
...
#---------------------------------------------------------------------------------------
    ...
    def add_ngram(self,wordsInSentence):
        for n in range(len(wordsInSentence),1,-1):#this would get me all possile gframs from 2 to the len of sentence
            for i in range(len(wordsInSentence) - n + 1):
                context = tuple(wordsInSentence[i:i + n - 1])
                next_word = wordsInSentence[i + n - 1]
                if context not in self.ngram:
                    self.ngram[context] = {}
                if next_word not in self.ngram[context]:
                    self.ngram[context][next_word] = 0
                self.ngram[context][next_word] += 1


if __name__=="__main__":
    with open("SpaceIpsum.txt", "r", encoding="utf-8") as f:
        text = f.read()
    # text="she hadn't"
    Report=SmartTextAnalyzer(text)
    print(Report.predict_next_topN(["no"]))
